...
{index:21,name:"MEDIA PLAYER",frequency:"91.4",videoId:"dPkzYz-AYOs"},
{index:22,name:"MOTOMAMI LOS SANTOS",frequency:"97.8",videoId:"30uA_Hppzpc"},
{index:23,name:"BLONDED LOS SANTOS",frequency:"104.6",videoId:"-tVumJBaTWY"}
];
let player,currentStationIndex=0,progressTimer=null,userSeeking=false;
const $=s=>document.querySelector(s),stationList=$("#station-list"),currentStation=$("#current-station"),currentTrack=$("#current-track"),frequency=$("#frequency"),playPause=$("#play-pause"),progress=$("#progress"),currentTime=$("#current-time"),duration=$("#duration"),volume=$("#volume"),muteButton=$("#mute");
...
function updateActiveStation(){const s=stations[currentStationIndex];document.querySelectorAll(".station-button").forEach((b,i)=>b.classList.toggle("active",i===currentStationIndex));currentStation.textContent=s.name;frequency.textContent=s.frequency;currentTrack.textContent="GTA RADIO — FULL STATION";document.title=`${s.name} · Skivback FM`}
